    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Internal documentation and guides</p>
        </div>
        {isExec && (
      ...
      {showCreate && isExec && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <form onSubmit={handleCreate} className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 space-y-4">
            <h2 className="text-sm font-medium text-gray-900">New Wiki Page</h2>
            ...
            <div>
              <label className="block text-[10px] text-gray-400 mb-1.5 uppercase tracking-wider">Role Tag (optional)</label>
            ...
            <div>
              <label className="block text-[10px] text-gray-400 mb-1.5 uppercase tracking-wider">Content (Markdown)</label>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-12 text-center text-sm text-gray-400">No wiki pages yet.</div>
      ) : (
        <div className="flex gap-6 items-start">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-4 sticky top-4">
              ...
              <div className="mb-4">
                <input
                  ...
                  className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-xs text-gray-900 focus:ring-1 focus:ring-gray-300 focus:border-gray-300 outline-none transition-all"
                />
              </div>
              ...
              <nav className="space-y-4">
                {CATEGORIES.map(cat => {
                  ...
                  return (
                    <div key={cat}>
                      <p className="text-[10px] text-gray-400 uppercase tracking-wider font-medium mb-1.5">{cat}</p>
                      <div className="space-y-0.5">
                        {catPages.map(p => (
                          <button
                            key={p.slug}
                            onClick={() => { setSelectedSlug(p.slug); setEditing(false); setShowPreview(false); }}
                            className={`w-full text-left px-2.5 py-1.5 rounded-md text-xs transition-all truncate ${selectedSlug === p.slug ? 'bg-gray-100 text-gray-900 font-medium' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50'}`}
                          >
                            {p.title}
                            {p.role_tag && (
                              <span className="ml-1.5 text-[9px] px-1.5 py-0.5 rounded-full bg-gray-100 text-gray-400 uppercase">{p.role_tag}</span>
                            )}
                          </button>
                        ))}
                      </div>
          ...
          <div className="md:hidden w-full mb-4">
            <input
              ...
              onChange={e => setSearch(e.target.value)}
              placeholder="Search pages..."
              className="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-xs text-gray-900 focus:ring-1 focus:ring-gray-300 focus:border-gray-300 outline-none transition-all mb-2"
            />
            <select
              value={selectedSlug || ''}
            ...
            {loadingPage ? (
              ...
              <form onSubmit={handleUpdate} className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
                <div className="flex items-center justify-between">
                  <h2 className="text-sm font-medium text-gray-900">Editing: {currentPage.title}</h2>
                  <div className="flex gap-1 bg-gray-100 rounded-lg p-1">
                    <button type="button" onClick={() => setShowPreview(false)} className={`px-3 py-1 rounded-md text-xs font-medium transition-all ${!showPreview ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500'}`}>Edit</button>
                    <button type="button" onClick={() => setShowPreview(true)} className={`px-3 py-1 rounded-md text-xs font-medium transition-all ${showPreview ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500'}`}>Preview</button>
                  </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                ...
                {showPreview ? (
                  <div className="prose prose-sm max-w-none border border-gray-200 rounded-lg p-4 min-h-[200px]">
                ...
                <div className="flex gap-3">
                  ...
                  <button type="button" onClick={() => { setEditing(false); setShowPreview(false); }} className="text-gray-500 text-[11px] uppercase tracking-[0.15em] font-semibold px-6 py-2.5 rounded-lg border border-gray-200 hover:border-gray-300 transition-all">Cancel</button>
                </div>
              </form>
            ) : (
              /* View mode */
              <div className="bg-white rounded-xl border border-gray-200">
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1.5">
                        <span className="text-[10px] px-2 py-0.5 rounded-full font-medium uppercase bg-gray-100 text-gray-500">{currentPage.category}</span>
                ...
                <div className="p-6 prose prose-sm max-w-none prose-headings:text-gray-900 prose-p:text-gray-600 prose-a:text-blue-600 prose-strong:text-gray-900 prose-code:text-gray-800 prose-code:bg-gray-100 prose-code:px-1 prose-code:rounded prose-pre:bg-gray-50 prose-pre:border prose-pre:border-gray-200 prose-table:text-sm prose-th:text-left prose-th:px-3 prose-th:py-2 prose-td:px-3 prose-td:py-2 prose-hr:border-gray-200">
