...
export default function PartnersManagePage() {
  ...
  const selectClass = 'w-full px-3 py-2.5 bg-white border border-gray-200 rounded-lg text-gray-900 text-sm focus:ring-1 focus:ring-gray-300 outline-none transition-all';

  if (!isExec) {
    return (
      <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
        <p className="text-gray-400 text-sm">You don&apos;t have permission to manage partners.</p>
      </div>
    );
  }
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage sponsors, affiliates, and partner organizations</p>
        </div>
        {!showForm && (
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-6 mb-6 space-y-4">
          <h2 className="text-sm font-medium text-gray-900">{editingId ? 'Edit' : 'New'} Partner</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-[10px] text-gray-400 mb-1.5 uppercase tracking-wider">Tier</label>
          ...
          <div>
            <label className="block text-[10px] text-gray-400 mb-1.5 uppercase tracking-wider">Description</label>
            <textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} rows={3} className={inputClass} />
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label className="block text-[10px] text-gray-400 mb-1.5 uppercase tracking-wider">Website</label>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
            ...
            <div>
              <label className="block text-[10px] text-gray-400 mb-1.5 uppercase tracking-wider">TikTok</label>
              <input type="text" value={form.tiktok} onChange={e => setForm({ ...form, tiktok: e.target.value })} placeholder="@handle" className={inputClass} />
            </div>
            <div>
              <label className="block text-[10px] text-gray-400 mb-1.5 uppercase tracking-wider">Twitter/X</label>
            ...
            <div>
              <label className="block text-[10px] text-gray-400 mb-1.5 uppercase tracking-wider">Facebook</label>
              <input type="text" value={form.facebook} onChange={e => setForm({ ...form, facebook: e.target.value })} className={inputClass} />
            </div>
            <div>
              <label className="block text-[10px] text-gray-400 mb-1.5 uppercase tracking-wider">YouTube</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-[10px] text-gray-400 mb-1.5 uppercase tracking-wider">Sort Order</label>
            ...
            <div className="flex items-end">
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-sm text-gray-700">Current partner</span>
              </label>
            </div>
          </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
          <p className="text-gray-400 text-sm mb-4">No partners yet.</p>
          <button onClick={() => setShowForm(true)} className="text-gray-600 text-xs hover:underline">Add your first partner</button>
        </div>
      ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100">
                  <th className="text-left text-[10px] text-gray-400 uppercase tracking-wider font-medium px-5 py-3">Partner</th>
                  <th className="text-left text-[10px] text-gray-400 uppercase tracking-wider font-medium px-5 py-3 hidden sm:table-cell">Category</th>
                  <th className="text-left text-[10px] text-gray-400 uppercase tracking-wider font-medium px-5 py-3">Tier</th>
                  <th className="text-left text-[10px] text-gray-400 uppercase tracking-wider font-medium px-5 py-3 hidden sm:table-cell">Status</th>
                  <th className="text-right text-[10px] text-gray-400 uppercase tracking-wider font-medium px-5 py-3">Actions</th>
                </tr>
              </thead>
              <tbody>
                {partners.map(p => (
                  <tr key={p.id} className={`border-b border-gray-50 hover:bg-gray-50/50 transition-colors ${!p.is_current ? 'opacity-50' : ''}`}>
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          {p.website_url && <p className="text-[11px] text-gray-400 truncate max-w-[200px]">{p.website_url}</p>}
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => toggleCurrent(p)} className="text-xs text-gray-400 hover:text-gray-600 transition-colors">
                          {p.is_current ? 'Mark Past' : 'Mark Current'}
                        </button>
                        <button onClick={() => startEdit(p)} className="text-xs text-gray-400 hover:text-gray-600 transition-colors">Edit</button>
