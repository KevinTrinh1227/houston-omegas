...
export default function QuickActions() {
  ...
  const linkClass = "flex items-center gap-3.5 px-3 py-2.5 rounded-xl text-white/60 hover:text-white hover:bg-white/[0.08] transition-all group";

  return (
    <div ref={ref} className="fixed bottom-6 right-6 z-50 hidden lg:block">
      {/* Panel */}
      <div className={`absolute bottom-16 right-0 w-80 bg-black/90 backdrop-blur-xl border border-white/[0.1] rounded-2xl overflow-hidden shadow-2xl transition-all duration-300 origin-bottom-right ${open ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 translate-y-2 pointer-events-none'}`}>
        ...
        <div className="px-2 pb-3">
          {navActions.map((a) => (
            <Link
              ...
              className={linkClass}
            >
              <div className="w-10 h-10 rounded-lg bg-white/[0.06] group-hover:bg-white/[0.1] flex items-center justify-center shrink-0 transition-colors">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-white/50 group-hover:text-white transition-colors">{a.icon}</svg>
              </div>
              <div>
          ...
          >
            <div className="w-10 h-10 rounded-lg bg-white/[0.06] group-hover:bg-white/[0.1] flex items-center justify-center shrink-0 transition-colors">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-white/50 group-hover:text-white transition-colors">
      ...
      <button
        ...
        className="relative w-12 h-12 rounded-full bg-black/80 backdrop-blur-md border border-white/[0.12] text-white flex items-center justify-center shadow-lg hover:bg-[#1a1a1a] transition-all duration-300"
